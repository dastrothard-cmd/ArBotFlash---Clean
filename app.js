...
const state={all:[],deck:[],index:0,revealed:false,quiz:false,voices:[],selectedVoice:null,speaking:false};
...
function render(){renderCard();$('#deckMeta').textContent=`${state.deck.length} card${state.deck.length===1?'':'s'} in this deck`;$('#stat-count').textContent=state.all.length;}
...
function getVoiceText(sp){
  const first=(bestCards(sp)[0]||{}).text||'';
  const second=(bestCards(sp)[1]||{}).text||'';
  return `${sp.commonName}. ${sp.scientificName}. ${first} ${second}`.trim();
}
function populateVoices(){
  ...
  state.voices=voices;
  const preferred=voices.find(v=>/en-AU/i.test(v.lang))||voices.find(v=>/Australia|Australian/i.test(v.name))||voices.find(v=>/^en-/i.test(v.lang))||voices[0];
  state.selectedVoice=state.selectedVoice||preferred||null;
  sel.innerHTML='';
  voices.filter(v=>/^en/i.test(v.lang)||/Australia|Australian|English/i.test(v.name)).forEach((v,i)=>{
    const o=document.createElement('option');
    o.value=v.name;
    o.textContent=`${v.name} (${v.lang})`;
    if(state.selectedVoice&&v.name===state.selectedVoice.name) o.selected=true;
    sel.appendChild(o);
  });
  if(!sel.children.length){
    const o=document.createElement('option');o.value='';o.textContent='Default browser voice';sel.appendChild(o);
  }
}
...
function speakCurrent(){
  if(!('speechSynthesis' in window)){setVoiceStatus('Voice unavailable');return;}
  const sp=state.deck[state.index%state.deck.length];
  if(!sp) return;
  if(state.speaking){stopSpeech();return;}
  window.speechSynthesis.cancel();
  const u=new SpeechSynthesisUtterance(getVoiceText(sp));
  if(state.selectedVoice) u.voice=state.selectedVoice;
  u.lang=(state.selectedVoice&&state.selectedVoice.lang)||'en-AU';
  u.rate=.86;
  u.pitch=.95;
  u.onstart=()=>{state.speaking=true;setVoiceStatus('Speaking');};
  u.onend=()=>{state.speaking=false;setVoiceStatus('Off');};
  u.onerror=()=>{state.speaking=false;setVoiceStatus('Stopped');};
